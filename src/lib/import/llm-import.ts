...
export async function extractRecipeDraftWithLlm(
  ...
  const prompt = [
    ...
    "2. Prep tasks: A list of tasks that can be done to process/combine raw ingredients into as few as possible 'prepared ingredients'. First, scan the whole recipe to identify ingredients that end up added to a pot/pan at the same time (look for expressions like 'add the garlic, ginger and soy sauce' or 'crumble in a stock cube with the boiled water'), and in each case aim to prepare and combine those into a single prepared ingredient in a prep task.  Prep tasks must avoid sequencing, timing, or cooking.  Don't include things in the same prep step if they don't get used in the recipe at the same moment.  Making salads, chopping and mixing veg, making stocks or spice mixes are good candidates for prep tasks.  At the end of the prep tasks the chef should have a set of bowls of of prepared ingredients ready to be added in the cooking steps.  Anything that is added to the pot or pan at the same time should be prepped together in the same prep task when possible.  A mini chopper machine is often used if ingredients like shallots, garlic, ginger or herbs need to be chopped and added together in the same cook step.",
    "3. Cooking steps: Ordered heat/time-sensitive steps that can't be done ahead of time, using raw ingredients and the specific 'prepared ingredients' prepped in the prep tasks.",
    "",
